 ...
 *
 * ## Cache purging
 * The cache can be purged by calling the static method purgeCache(). It is recommended that you call this either during
 * a cron job or according to a request lottery. The method will remove all cache entries that haven't been used for
 * a given period of time, which by default is 30 minutes.
 *
 ...
 * _atime_ attribute is not automatically updated on file access as a way of reducing file access overhead. In such
 ...
 * setRowChecked() and reports back to the browser whether or not the attempt to (un)tick the row succeeded.
 *
 * ## Row grouping
...
class ResultsPager extends PageElement
{
	...
	/** @var array The callbacks for row data. */
	private array $m_rowCallbacks = [];

	/** @var array The callbacks for header row data. */
	private array $m_headerRowCallbacks = [];
	 ...
	 *
	 * When called, all the metadata files (resultspager-*.meta) in the cache directory have their last access time
     * checked against the expiry duration. If the file was last accessed more than the specified number of minutes ago,
     * it is assumed to be no longer needed and the results are purged from the cache.
	 *
	...
	{
        assert (0 < $expiry, new Exception("Invalid expiry duration {$expiry}."));
        ...
        foreach (new DirectoryIterator(self::cacheDirectory()) as $file) {
            ...
            if (!$file->isFile()) {
                AppLog::error("Unexpected entry in ResultsPager cache directory: '{$file->getPathname()}'", __FILE__, __LINE__, __FUNCTION__);
                continue;
            }
            ...
            if (!preg_match("/resultspager-([0-9a-f]{32})\\.meta$/", $filePath, $captures)) {
                continue;
            }
            ...
            if (false === $fileTime) {
                AppLog::error("Could not determine last access time for ResultsPager cache file: '{$file->getPathname()}'", __FILE__, __LINE__, __FUNCTION__);
					continue;
				}
     ...
     */
    protected static function removeCachedResults(string $resultsId): void
    {
        foreach (glob(static::cacheDirectory() . "/resultspager-{$resultsId}.*", GLOB_NOSORT) as $cacheFilePath) {
            if (!@unlink($cacheFilePath)) {
                AppLog::error("Could not delete ResultsPager cache file: '$cacheFilePath'", __FILE__, __LINE__, __FUNCTION__);
			}
		}
	}
     ...
     *
     * All callbacks should expect to receive the following parameters, in this order:
     * - **$row** array[string->mixed] A the row data to process.
     * - **$mimeType** _string_ The MIME type being used for output.
     *
     * Callbacks should process the row and return an array of processed data. The array should remain composed entirely
     * of values that can be coerced into strings (preferably actually strings). If the callback does not want to
     * process the row, it should return it unmodified. Using this is preferable to using column callbacks when:
     * - you want to modify multiple columns in each row, depending on the data in the whole row
     * - you want to add or remove columns
     *
     * Note that callbacks are invoked as variable functions, not by using call_user_func(). This means that callbacks
     * of the following forms are not supported (even though they satisfy _callable_):
     * - ['MyClass', 'parent::myMethod']
     ...
     * @param $callback callable The callback to use.
     */
    public function addRowCallback(callable $callback): void
    {
        $this->m_rowCallbacks[] = $callback;
    }
    ...
    protected function doRowCallbacks(array $row, string $mimeType = "text/html"): array
    {
        if (empty($this->m_rowCallbacks)) {
            return $row;
	}

        foreach ($this->m_rowCallbacks as $callback) {
            if (is_callable($callback, false)) {
                $processedRow = $callback($row, $mimeType);

                if (is_array($processedRow)) {
                    $row = $processedRow;
                } else {
                    AppLog::error("row callback " . stringify($callback) . " did not return an array", __FILE__, __LINE__, __FUNCTION__);
                }
            } else {
                AppLog::error("found uncallable row callback: " . stringify($callback), __FILE__, __LINE__, __FUNCTION__);
            }
		}
    ...
    protected function doHeaderRowCallbacks(array $headerRow, string $mimeType = "text/html"): array
    {
        if (empty($this->m_headerRowCallbacks)) {
            return $headerRow;
				}

        foreach ($this->m_headerRowCallbacks as $callback) {
            if (is_callable($callback, false)) {
                $processedHeaderRow = $callback($headerRow, $mimeType);

                if (is_array($processedHeaderRow)) {
                    $headerRow = $processedHeaderRow;
                } else {
                    AppLog::error("header row callback " . stringify($callback) . " did not return an array", __FILE__, __LINE__, __FUNCTION__);
                }
            } else {
                AppLog::error("found uncallable header row callback: " . stringify($callback), __FILE__, __LINE__, __FUNCTION__);
            }
        }
	 ...
	 * @param $className string|null Force the pager to be of a specific class, if it is a subclass of this base
	...
	public static function cacheDirectory(): string
	{
		...
		if(is_null($s_dir)) {
			$s_dir = Application::instance()->rootDir() . "/" . Application::instance()->config("app.cache.dir", "cache") . "/equit-resultspager";
	...
	public function html(int $flags = self::DefaultOutputFlags, array $options = []): string
	{
		...
		if($start < $end) {
			...
			if(!($flags & self::NoColumnTitles)) {
                ...
                $headerRow = $this->doRowCallbacks($data[$start % self::ResultsCacheFileChunkSize], "text/html");
			...
			for($i = $start; $i < $end; ++$i) {
                ...
                $rowData = $this->doRowCallbacks($data[$dataRowIndex], "text/html");
