...
/**
 * Validator rule to ensure that some data is non-empty if some other field is set to one of a given set of values.
 */
class RequiredIf implements DatasetAwareRule
{
    use KnowsDataset;
     ...
     * @param array $values The set of values that the other field can have to make the tested data required.
     */
    ...
    public function setOtherFieldValues(array $values): void
    {
        ...
        8 <= PHP_MAJOR_VERSION
            ? new LogicException("Must have at least one value to use RequiredIf rule.")
            : "Must have at least one value to use RequiredIf rule."
        ));
     ...
     *
     * The data passes the rule if the related field does not have one of the specified values or if the data tested is
     * not empty.
     *
