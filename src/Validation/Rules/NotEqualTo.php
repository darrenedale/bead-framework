...
/**
 * Validation rule that ensures a value is not equal to the value in another field in the dataset.
 *
 ...
 *
 * - if the data is an int, or can be coerced to an int, it must not be equal to the int value in the referenced field.
 * - if the data is aa float, or can be coerced to a float, it must not be equal to the float value in the referenced
 *   field.
 ...
 */
class NotEqualTo extends FieldComparingRule
{
    /**
    ...
    {
        return $data !== $this->otherValueAsInt();
    }
    ...
    {
        return tr("The %1 field must not be equal to the value in the %2 field.", __FILE__, __LINE__, $field, $this->otherField());
    }
}
