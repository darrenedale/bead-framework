...
class FunctionReplacement
{
    ...
    /** @var array<string,FunctionReplacement[]>  */
    ...
    {
        $expecation = new Expectation(Expectation::OutcomeValue, null, ...$arguments);
     ...
     * Get the key into the register of installed mocks for a mock.
     *
     * @param FunctionReplacement $mock The mock whose key is sought.
     *
     * @return string The key.
     */
    private static function keyForMock(FunctionReplacement $mock): string
    {
     ...
     *
     * @param FunctionReplacement $mock
     */
    private static function activateMock(FunctionReplacement $mock): void
    {
        if ($mock->isMethod()) {
     ...
     * that function.
     *
     * @param FunctionReplacement $mock The mock to install.
     */
    public static function installMock(FunctionReplacement $mock): void
    {
     ...
     * longer mocked.
     *
     * @param FunctionReplacement $mock The mock to remove.
     *
     * @return void
     */
    public static function removeMock(FunctionReplacement $mock): void
    {
        $key = self::keyForMock($mock);
        $idx = array_search($mock, self::$m_mocks[$key] ?? []);
     ...
     *
     * @param FunctionReplacement $mock The mock to look for.
     *
     * @return bool `true` if the mock is in the stack for the function it doubles for, `false` if not.
     */
    public static function mockIsInstalled(FunctionReplacement $mock): bool
    {
        $key = self::keyForMock($mock);
     ...
     */
    public static function mockIsTop(FunctionReplacement $mock): bool
    {
        $key = self::keyForMock($mock);
     ...
     * `false` if not.
     */
    public static function mockIsActive(FunctionReplacement $mock): bool
    {
     ...
     *
     * @return FunctionReplacement|null The active mock, or `null` if no mock is active.
     */
    public static function activeMock(string $functionOrClass, ?string $methodName = null): ?FunctionReplacement
    {
        $mock = self::topMock($functionOrClass, $methodName);
        return (isset($mock) && $mock->isActive()) ? $mock : null;
    }

    public static function topMock(string $functionOrClass, ?string $methodName = null): ?FunctionReplacement
    {
        $key = self::keyForClassAndFunction($functionOrClass, $methodName);
